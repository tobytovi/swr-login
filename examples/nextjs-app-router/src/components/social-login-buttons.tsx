...
export function SocialLoginButtons() {
  const { login: googleLogin, isLoading: isGoogleLoading } = useLogin('oauth-google');
  const { login: githubLogin, isLoading: isGithubLoading } = useLogin('oauth-github');
  const { login: passkeyLogin, isLoading: isPasskeyLoading } = useLogin('passkey');
  ...
  return (
    <div className="space-y-3">
      ...
      <button
        ...
        className="w-full flex items-center justify-center gap-3 py-2.5 px-4 bg-white hover:bg-gray-50 border border-gray-300 rounded-lg font-medium text-gray-700 transition disabled:opacity-50"
      >
